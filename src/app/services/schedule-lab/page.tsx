...
import { Calendar, Clock, FileText, Phone, CheckCircle, Upload, X } from "lucide-react";
...
export default function ScheduleLabPage() {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        if (!doctorRequestFile) {
            setCustomAlert({
                isOpen: true,
                type: 'warning',
                title: 'Missing Doctor\'s Request',
                message: 'Please upload a doctor\'s request or prescription image. This is required for all lab appointments.',
            });
            return;
        }
    ...
    const handleConfirmSubmission = async () => {
        ...
        try {
            ...
            try {
                doctorRequestImageAsset = await uploadImageToSanity(doctorRequestFile as File);
            } catch (error) {
                setCustomAlert({
                    isOpen: true,
                    type: 'error',
            ...
            setCustomAlert({
                ...
                action: () => {
                    // This action can be used to redirect or clear the form if needed
            ...
            let errorMessage = 'There was an unexpected error scheduling your appointment. Please try again or contact us directly.';
            let errorTitle = 'Submission Failed';

            if (error instanceof Error) {
                if (error.message.includes('Insufficient permissions')) {
                    errorMessage = 'Unable to schedule appointment due to permissions. Please contact us directly at (02) 8825-5236.';
                } else if (error.message.includes('authentication')) {
                    errorMessage = 'Authentication error. Please refresh the page and try again.';
                } else {
                    errorMessage = `Error: ${error.message}. Please try again or contact us directly.`;
                }
            }
                            ...
                            <Card className="sticky top-4">
                                ...
                                <CardContent>
                                    <form onSubmit={handleSubmit} className="space-y-4">
                                        ...
                                        <div className="border-t pt-4">
                                            ...
                                            <div className="space-y-3">
                                                        ...
                                                        {!doctorRequestPreview ? (
                                                            <div className="border-2 border-dashed border-red-300 rounded-lg p-4 text-center hover:border-red-400 transition-colors">
                                                                <input
                                                                    ...
                                                                    required
                                                                />
                                                                <label htmlFor="doctorRequest" className="cursor-pointer text-sm font-medium text-red-600">
                                                            ...
                                                            <div className="relative w-full h-40 border rounded-lg overflow-hidden group">
                                                                ...
                                                                <button
                                                                    ...
                                                                    className="absolute top-1 right-1 p-1 rounded-full bg-black/50 text-white hover:bg-black transition-colors"
                                                                >
                                                                    <X className="h-4 w-4" />
                                                                </button>
                                                            </div>
                                                ...
                                                <div className="hidden">
                                                    <Label htmlFor="doctorRequestNotes">Notes on Doctor's Request (Optional)</Label>
                                                    <Textarea
                                        ...
                                        <Button type="submit" className="w-full" disabled={submitting || selectedTests.length === 0}>
                                            {submitting ? 'Submitting...' : 'Submit Appointment'}
                                        </Button>
                                        <div className="mt-4">
                                            <p className="text-xs text-muted-foreground">
                                                By clicking "Submit Appointment", you agree to our <Link href="/terms-of-service" className="underline">Terms of Service</Link> and <Link href="/privacy-policy" className="underline">Privacy Policy</Link>.
                                            </p>
                                        </div>
                                    </form>
                                </CardContent>
